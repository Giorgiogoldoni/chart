...
def calc_signal(i, closes, kama_f, er, sar_b, ao, scores):
    if i < 35: return 'NONE'
    kfv = kama_f[i]; erv = er[i]; erp = er[i-1]; aov = ao[i]; aop = ao[i-1]
    sb = sar_b[i]; sc = scores[i]; scp = scores[i-1]
    if kfv is None or erv is None or aov is None: return 'NONE'
    p = closes[i]; pp = closes[i-1]
    above_kf  = p > kfv
    ...
    if not sb or (not ao_pos and sc < 65): return 'EXIT1'
    if sb and ao_pos and above_kf and sc >= 65 and er_grow and sc_grow and erv > 0.6: return 'BUY3'
    if sb and ao_pos and above_kf and sc >= 65 and er_grow: return 'BUY2'
    if above_kf and erv > 0.4 and ao_grow and sb: return 'BUY1'
    return 'WATCH'
...
def simulate_trades(bars, signals):
    ...
    for i, (bar, sig) in enumerate(zip(bars, signals)):
        ...
        if not in_trade:
            ...
            days += 1
            time_stop = ent_sig=='BUY1' and days>=7 and sig not in ('BUY2','BUY3')
            is_exit = sig in ('EXIT1','EXIT2','EXIT3') or time_stop or i==len(bars)-1
            if is_exit:
                exit_sig = 'EXIT1a' if time_stop else ('OPEN' if (i==len(bars)-1 and sig not in ('EXIT1','EXIT2','EXIT3')) else sig)
...
def process_ticker(entry):
    ticker = entry['ticker']
    name   = entry['name']
    # Yahoo Finance Europa: aggiungi .MI se non ha estensione
    yf_symbol = ticker + '.MI' if '.' not in ticker else ticker

    try:
        df = yf.download(yf_symbol, period=PERIOD, interval=INTERVAL,
                         progress=False, auto_adjust=True, timeout=20)
        if df is None or len(df) < 30:
            log.warning(f"  {ticker}: dati insufficienti ({len(df) if df is not None else 0} barre)")
            return None
        ...
        signals = [calc_signal(i, closes, kama_f, er, sar_b, ao, scores) for i in range(len(closes))]
